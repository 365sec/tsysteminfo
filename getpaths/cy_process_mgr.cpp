...
void   CProgressMgr::PrintAll()
{
	
	for ( DWORD nIndex = 0; nIndex < m_tasks.size(); nIndex++)
	{
		OutputDebugString(m_tasks[nIndex].szFullPath);
		OutputDebugString(L"------\n");
...
BOOL CProgressMgr::KillByDir(LPCTSTR dir,LPCTSTR ignore){
	BOOL  bRet = TRUE;
	if ( m_tasks.size() == 0 )
			return bRet;
	...
	for ( DWORD nIndex = 0; nIndex < m_tasks.size(); nIndex++)
	{
		if ( m_tasks[nIndex].nProcId != 0
			&& _tcsnccmp(m_tasks[nIndex].szFullPath, dir,_tcslen(dir)) == 0
			&& _tcsnccmp(m_tasks[nIndex].szFullPath, ignore,_tcslen(ignore)) != 0 )
		{
			OutputDebugString(L"\nkill------");
			OutputDebugString(m_tasks[nIndex].szFullPath);
		    OutputDebugString(L"------\n");
			if(KillTask(nIndex, TRUE) == FALSE){
				bRet = FALSE;
			}
		}
	}
...
BOOL CProgressMgr::GetFullPath( LPCTSTR lpPath, wstring& strFullPath )
{
	if ( m_tasks.size() == 0 )
		return FALSE;
